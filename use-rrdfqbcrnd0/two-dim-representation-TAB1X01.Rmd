...
```{r, eval=TRUE}
dataCubeFile<- file.path("..", "res-ttl", "CDISC-pilot-TAB1X01.ttl")
...
The following query is all to long - and can be made simpler and more readable.

```{r, eval=TRUE}

(forsparqlprefix.lines<- unlist(strsplit(forsparqlprefix,"\n")))
(useit<- grepl("prefix ds:", forsparqlprefix.lines ))
(forsparqlprefix.no.ds<- paste(forsparqlprefix.lines[!useit],collapse="\n",sep="")) ## remove the ds: prefix, so the URI is sull


## 

cube.res.rq<-  paste( forsparqlprefix.no.ds,
'
select
 ...
 ?compflVarLabel    ?compflLevelLabel    ?compfl
...
?compfl skos:prefLabel    ?compflLevelLabel .
...
[] qb:codeList [ skos:hasTopConcept ?compfl ] ;
   rdfs:label ?compflVarLabel .
...
(code:factor-quantity code:procedure-count code:factor-proportion code:procedure-percent)
 ...
 ?compfl = code:compfl-Y
 )
}
order by
?ittflLevelLabel ?safflLevelLabel ?effflLevelLabel ?comp24flLevelLabel ?compflLevelLabel
',
"\n"                               
)
...
cat( cube.res.rq, file=file.path("..", "sparql-rq", "tab1x01.rq"), sep="\n")

## knitr::kable(cube.res)


varlabels<- cube.res[1,c("ittflVarLabel",   "safflVarLabel",   "effflVarLabel",   "comp24flVarLabel",   "compflVarLabel"   )]
varorder<- apply( cube.res[,c("ittflLevelLabel", "safflLevelLabel", "effflLevelLabel", "comp24flLevelLabel", "compflLevelLabel"  )]=="Y",1, which)

present.res<- cbind(
    rowlabel=varlabels[varorder],
